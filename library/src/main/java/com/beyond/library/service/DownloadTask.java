 ...
 * <p/>
 * <p/>
 * 下载任务，一个文件对应一个DownloadTask，一个DownloadTask对应多个DownloadThread
 */
public class DownloadTask {
    ...
    private int mPausedThreadCount = 0;
    ...
    public synchronized void doDownload() {
        mFinished = 0;
        mPausedThreadCount = 0;
        // 读取数据库的线程信息
        List<ThreadInfo> threads = mDao.getThreads(mFileInfo.getUrl());
        if (threads.size() == 0) {// 首次下载,创建线程实体类,保存到SQLite
            // 计算每个线程下载的长度
            ...
            for (int i = 0; i < mThreadCount; i++) {
                ...
                ThreadInfo threadInfo = new ThreadInfo(i, mFileInfo.getUrl(), start, end, 0, mFileInfo.getLength());
        ...
        for (ThreadInfo threadInfo : threads) {
            ...
            mFileInfo.setAcceptRanges(threadInfo.getAcceptRange() == 0);
...
//		mDao.updateThread(threadInfo.getUrl(), threadInfo.getId(), threadInfo.getFinished(),0);
        mPausedThreadCount++;
        if (mPausedThreadCount == mThreadMap.size()) {
